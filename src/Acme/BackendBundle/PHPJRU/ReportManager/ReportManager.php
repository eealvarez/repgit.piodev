...
class ReportManager
{
	...
	* */
	...
	private $typeExtension = 'ReportExtension';
        ...
        private $connetionDefalt = null;
        
        public function getConnetionDefalt() {
            return $this->connetionDefalt;
        }

        public function setConnetionDefalt($connetionDefalt) {
            $this->connetionDefalt = $connetionDefalt;
        }      
	...
	*/
	 ...
	 * */
	public function RunToBuffer($extensionName, $type, $container = null)
	{
		$file = $this->RunToFile($extensionName, $type, $container);
	 ...
	 * */
	public function RunToFile($extensionName, $type, $container = null)
	{
		$extension = null;
		...
		$conn = $extension->getConexion();
 		...
 		if(! $conn)
 		{
                    if($this->connetionDefalt === null){
                       $conn = PJRUConexion::get(); 
                    }
                    else {
                        $conn = $this->connetionDefalt;
                    }
 		}	
		...
		$fecha = new DateTime();
                ...
                if($user instanceof Acme\BackendBundle\Entity\User){
                    $username = $user->getUsername();
                }
		$outfilename = $this->reportOutDir. "\\DOC\\" . $username . "_" . $fecha->format('Y.m.d_H.i.s') . "_" . $reportName;
		...
		if(get_class($conn) == 'JdbcConnection'){
                ...
                $inputFileName = realpath($this->extensionFolder).'\\'.$reportFileName;
		$result = $report->$methodName($inputFileName,$outfilename,$parameters,$sqlSentence,$conn);
